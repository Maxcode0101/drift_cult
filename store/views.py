...
# Product list view with optional search and category filter
...
# Render detail view of a product, with redirect if POST (e.g. adding to cart)
...
# Authenticated user profile view showing order history
@login_required
def profile_view(request):
    orders = Order.objects.filter(user=request.user, is_paid=True).order_by(
        '-created_at')
    return render(request, 'store/profile.html', {'orders': orders})

# View for shopping cart contents
@login_required
def view_cart(request):
    cart_items = CartItem.objects.filter(user=request.user)
    total = sum(
        item.product_size.product.price * item.quantity for item in cart_items)
    return render(
...
# Update cart quantity or remove item if quantity = 0
@login_required
...
# Checkout page showing cart total and Stripe public key
@login_required
...
# Create Stripe checkout session, store order and redirect user
@login_required
    ...
    for item in cart_items:
        ...
        line_items.append({
            'price_data': {
                ...
                'product_data': {
                    'name': f"{item.product_size.product.name}"
...
def order_confirmation(request):
    order = Order.objects.filter(
        user=request.user, is_paid=True).order_by('-created_at').first()
    ...
    total = sum(
        (
            item.product_size.product.price * item.quantity
...
# Save the commented code to a file for the user to copy
@csrf_exempt
...
# Updated version of newsletter signup with a rich HTML welcome email
@csrf_exempt
def newsletter_signup_ajax(request):
    if request.method == 'POST':
        email = request.POST.get('email')
        if email:
            ...
            if created:
                ...
                return JsonResponse({
                    'success': True,
                    'message': '✅ Welcome to the cult – check your inbox!'})

            return JsonResponse({
                'success': False,
                'message': '⚠️ You’re already subscribed.'
...
# Return robots.txt with search engine crawler directives
...
def add_product_sizes(request, product_id):
    ...
    SizeFormSet = modelformset_factory(ProductSize, fields=(
        'size', 'stock'), extra=6, can_delete=True)
    ...
    return render(request, 'store/add_product_sizes.html', {
        'formset': formset, 'product': product})
...
def admin_order_detail(request, order_id):
    ...
    if request.method == 'POST':
        ...
        if new_status in dict(Order.STATUS_CHOICES):
            ...
            messages.success(
                request, f"Order status updated to {new_status.title()}")
...
def admin_order_delete(request, order_id):
    ...
    return render(
        request, 'store/admin_order_confirm_delete.html',
        {'order': order})
...
def bulk_order_action(request):
    if request.method == 'POST':
            ...
            if action == 'delete':
                ...
                if order.status != action:
                    ...
                    if previous_status == 'processing' and action == 'shipped':
                        subject = (
                            f"📦 Your Drift Cult Order #{order.id} Has Shipped"
                        )
                        from_email = 'noreply@driftcult.art'
                        to_email = [order.user.email]
                        context = {'user': order.user, 'order': order}
                        html_message = render_to_string(
                            'emails/order_shipped.html', context)
        ...
        if action == 'delete':
            ...
            messages.success(
                request, f"{affected} orders updated to {action}.")
